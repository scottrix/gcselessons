#!/usr/bin/env python3
"""Regenerate ALL topic pages with enhanced content - handles all filename patterns."""
    ...
    for root, dirs, files in os.walk(os.path.join(BASE, "topics")):
        for f in files:
            if f.endswith('.html'):
                ...
                # Try multiple patterns to extract topic_id and topic_name
                topic_id = None
                topic_name = None
                
                # Pattern 1: A-Z\d+-
                m = re.match(r'([A-Z]\d+)-(.+)\.html$', f)
                if m:
                    topic_id, topic_name_kebab = m.groups()
                    topic_name = topic_name_kebab.replace('-', ' ').title()
                
                # Pattern 2: A-Z{2}\d+-
                if not topic_id:
                    m = re.match(r'([A-Z]{2}\d+)-(.+)\.html$', f)
                    if m:
                        topic_id, topic_name_kebab = m.groups()
                        topic_name = topic_name_kebab.replace('-', ' ').title()
                
                # Pattern 3: A-Z\d- (single digit)
                if not topic_id:
                    m = re.match(r'([A-Z]\d)-(.+)\.html$', f)
                    if m:
                        topic_id, topic_name_kebab = m.groups()
                ...
                # Pattern 4: filename without ID prefix - use filename as topic_name
                if not topic_id:
                    topic_name = f.replace('.html', '').replace('-', ' ').title()
                    topic_id = "L1"  # Default

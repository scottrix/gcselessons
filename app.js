...
function topicCard(topic) {
  ...
  return `
    <a href="${escapeHtml(topic.url)}" class="subject-card topic-result">
      ...
      <div class="paper-count">Lesson Plan</div>
    </a>`;
}
...
function runSearch(query) {
  ...
  if (subjects.length === 0 && topics.length === 0) {
    grid.innerHTML =
      `<p style="grid-column:1/-1;text-align:center;color:var(--text-secondary);">
         No results for "${escapeHtml(query)}". Try a subject name or lesson keyword.
       </p>`;
    return;
  }
  ...
  if (topicSubset.length > 0) {
    ...
    const header = extraCount > 0
      ? `Lessons (${topics.length} — showing first ${topicSubset.length})`
...
function renderAutocomplete(query) {
  ...
  el.innerHTML = items.map((item, i) => {
    ...
    const typeLabel = item.type === 'subject'
      ...
      : `${escapeHtml(item.context || '')} · Lesson`;
...
function loadTheme() {
  const savedTheme = localStorage.getItem('gcselessons-theme');
  const icon = document.getElementById('theme-toggle');
  if (savedTheme === 'light') {
